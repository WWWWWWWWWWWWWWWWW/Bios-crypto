...
\newcommand{\mysection}[1]    % Re-define the chaptering command to use
	{                   % THESE headers.
	\section{#1}
   \markboth{\textsf{www.libtomcrypt.com}}{\thesection ~ {#1}}
	}

\newcommand{\mystarsection}[1]    % Re-define the chaptering command to use
	{                   % THESE headers.
	\section*{#1}
   \markboth{\textsf{www.libtomcrypt.com}}{{#1}}
	}
\pagestyle{empty}
\begin{document}
\frontmatter
...
/** Accelerators **/
   ...
   */
   ...
   /** Accelerated CCM packet (one-shot)
       ...
       @param taglen     [in/out] Initial/final size of the tag
       @param direction  CCM_ENCRYPT or CCM_DECRYPT
       @return CRYPT_OK if successful
   */
   ...
   /** Accelerated GCM packet (one shot)
       ...
       @param adata      The additional authentication data
       ...
       @param direction  GCM_ENCRYPT or GCM_DECRYPT
...
int pmac_done(   pmac_state *state, 
              ...
              unsigned long *outlen);
\end{verbatim}
This will store upto \textit{outlen} bytes of the tag for the given \textit{state} into \textit{out}.  Note that if \textit{outlen} is larger
...
by using a cipher in CBC mode.  It also uses a single key which it expands into the requisit three keys for the MAC function.  A XCBC--MAC state is
...
Note: the output of this function is zero--padded as per PKCS \#1 specification.  This allows this routine to interoprate with PKCS \#1 padding functions properly.
...
int rsa_decrypt_key(
    ...
    const unsigned char *lparam, 
          unsigned long  lparamlen,
                ...
                rsa_key *key);
\end{verbatim}
This function will RSA decrypt \textit{in} of length \textit{inlen} then OAEP depad the resulting data and store it in
...
\index{FP\_LUT}
The optimization uses a window on the multiplicand of FP\_LUT bits (default: 8, min: 2, max: 12) and controls the memory/time trade-off. The larger the 
...
LibTomCrypt uses it's own format for ECC public and private keys.  While ANSI X9.63 partially specifies key formats (it covers public keys) it does it in a less
...
The \textit{keysize} is the size of the modulus in bytes desired.  Currently directly supported values are 24, 28, 32, 48 and 65 bytes which
...
This will import the ECC key from \textit{in} and store it in the ecc\_key structure \textit{key}.  If the operation fails it will free
...
int ecc_shared_secret(      ecc_key *private_key, 
                      ...
                      unsigned long *outlen);
\end{verbatim}
The \textit{private\_key} is your own key and \textit{public\_key} is the key the other user sent you.   Note that this function stores only the 
...
Where \textit{in} is an input symmetric key of no more than 64 bytes.  This function creates a random public key
...
data is placed in \textit{out} by \textit{ecc\_encrypt\_key()}.   The hash chosen must produce a message digest at least as large
as the symmetric key you are trying to share.

The data is encrypted to the public ECC \textit{key} such that only the holder of the private key can decrypt the payload.  If you want
...
This function will EC--DSA sign the message digest stored in the buffer \textit{in} of length inlen octets.  The signature
will be stored in the \textit{out} buffer of length \textit{outlen}.  The function requires a properly seeded PRNG and 
...
This function will verify the EC-DSA signature in \textit{sig} of length \textit{siglen} against the message digest \textit{hash}.  
It will store a non--zero value in \textit{stat} if the signature is valid.  Note that the function will not return
an error if the signature is invalid.  It will if the actual signature payload is an invalid format.  They ECC \textit{key}
must be the public (or private) ECC key corresponding to the key that performed the signature.

\subsection{Signature Format}
The signature code is an implementation of X9.62 EC--DSA and the output is conformant for GF(p) curves.
...
reduce the bandwidth of a signature.  For example, to have \textit{80-bits of security} with ElGamal you need a group of 
...
int dsa_decrypt_key(const unsigned char *in,  
                          ...
                          unsigned long *outlen, 
                                dsa_key *key);
\end{verbatim}
This will decrypt the ciphertext \textit{in} of length \textit{inlen} and store the original payload in \textit{out} of length \textit{outlen}.  The DSA \textit{key} must be a private key.

\mysection{Import and Export}
...
It's ideal that you cast the \textit{size} values to unsigned long to ensure that the proper data type is passed to the function.  Constants such as \textit{1} without
...
This will encode the list of ASN.1 objects in \textit{list} of length \textit{inlen} objects and store the output in \textit{out} of length \textit{outlen} bytes.  The function
...
\begin{verbatim}
int der_encode_integer(       mp_int *num, 
                       unsigned char *out, 
                       unsigned long *outlen);
...
int der_length_integer(       mp_int *num, 
                       unsigned long *len);
\end{verbatim}

These will encode or decode a signed INTEGER data type using the \textit{mp\_int} data type to store the large INTEGER.  To encode smaller values without allocating
an mp\_int to store the value the \textit{short} INTEGER functions were made available.
...
This will decode the input in the \textit{in} field of length \textit{inlen}.  It uses the provided ASN.1 list specified in the \textit{list} field which has \textit{outlen} elements.  
The \textit{inlen} field will be updated with the length of the decoded data type as well as the respective entry in the \textit{list} field will have the \textit{used} flag 
...
The function will terminate when either it hits an invalid ASN.1 type octet or it reads \textit{inlen} octets.  An early terminate is a soft error and returns
...
The output of length upto \textit{outlen} is stored in \textit{out}.  If \textit{outlen} is initially larger than the size of the hash functions output
...
on the password.  The \textit{hash\_idx} is the index of the hash you wish to use in the descriptor table.   The output of length upto 
...
does provide two RNG sources that will address the largest amount of end users as possible.  The \textit{sprng} PRNG provided 
...
such as register\_cipher() are not thread safe no matter what you do.  Its best to call them from your programs initialization
...
possible as some compilers may ignore the \textit{volatile} keyword or have multiple processors.  Provide your host application
...
Will build and install the libraries with the \textit{gmake} tool.  Similarly

\begin{verbatim}
CC=arm-gcc AR=arm-ar make 
\end{verbatim}

Will build the library using \textit{arm--gcc} as the compiler and \textit{arm--ar} as the archiver.
...
\mysection{tomcrypt\_cfg.h}
The file \textit{tomcrypt\_cfg.h} is what lets you control various high level macros which control the behaviour 
of the library. 
...
There are also options you can specify from the configure script or \textit{tomcrypt\_custom.h}.  
...
descriptors for the respective library are built and included in the library as \textit{ltm\_desc} or \textit{tfm\_desc} respectively.
...
Inside the \textit{symmetric\_key} union there is a \textit{void *data} which you can use to allocate data if you need a data structure that doesn't fit with the existing
...
\subsection{Testing}
The test() function is used to self--test the \textit{device}.  It takes no arguments and returns \textbf{CRYPT\_OK} if all is working properly.
